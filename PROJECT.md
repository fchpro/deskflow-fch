...
- Build while current app binaries are running: configure `-DCMAKE_RUNTIME_OUTPUT_DIRECTORY=<absolute-alternate-output>` then build normally. Item 02 validation uses `build/bin-streaming-validation`; normal `build/bin` is still running the prior app. Reset to `<project>/build/bin` when the user closes the running app. This cache option defaults to `<build>/bin`.
