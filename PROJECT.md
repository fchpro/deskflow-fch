...
- Native capture: WinRT WGC/D3D11 on Windows; ScreenCaptureKit builds natively on Mac with capture acceptance pending; Linux portal/PipeWire/XComposite awaits native build/acceptance. See `docs/project/streaming.md`.
...
Mac command: `QT_QPA_PLATFORM=offscreen ctest --test-dir build/src/unittests -LE streaming-extended --output-on-failure` after `build-validation`. Preserve/restore the general clipboard around this command. Current Mac result40/48 in91.21s is failing and over budget; see `docs/project/macos-validation.md`. The commands below describe Windows.
...
Unit tests (Qt Test): 62 C++ test binaries plus one Python package suite / 66 CTest registrations with `BUILD_STREAMING=ON` (25 upstream + 37 fork C++ binaries) under `src/unittests`; includes StreamingControlBoundaryTests, StreamingControlTests, StreamingControlLifecycleTests, StreamingControlHookTests and the extended StreamingControlTransportTests. Run via `ctest` from `build\src\unittests` (see Quick Check for the required PATH). No external master-test project. Previously completed streaming tests remain protected master tests.

## Architecture and Workflow Notes

- Taskbar reopening (2026-09-22): MainWindow drains and releases duplicate-launch local sockets before restoring the window, preventing pending-queue exhaustion. `InstanceActivationTests` joins quick/exhaustive checks (`ctest --test-dir build/src/unittests -R InstanceActivationTests --output-on-failure` with the documented PATH). Build and focused suite pass; hidden quick58/60 in38.09s retains foreground/audio failures and exceeds the30s target. Installed at existing `dist/2026-09-16`; backup `.ltemp/package-before-taskbar-activation`.100 installed relaunches retained one GUI process; user reported reopening works. See customizations section6.
- Streaming acceptance (2026-09-21 product decision): MainWindow enables automatic acceptance of new authenticated offers after opening the viewer. Windows/macOS use their enumerated native default audio output; missing defaults require output selection. Stop and separate desktop-control grants remain. Embedded launchers/viewers default to manual acceptance. Windows update installed at the existing dist/2026-09-16 path. Mac report: signed build/TLS restored; acceptance7/7, quick39/47; production worker tests refuse the absent lock-state key on macOS26.5.1, but installed-app stream failure was not directly reproduced. User confirmed Mac screenshot transfer into Windows Paint/Claude. ChatGPT paste remains unresolved; see the native bitmap publication note below. Capture teardown follow-up is installed on both systems; native Mac launcher teardown and lifetime checks now pass. See the coordinated Mac follow-up below for current failures. See `docs/tasks/stream-acceptance-and-mac-clipboard.md` for backups and evidence.
...
- Windows V5 clipboard publication: complete V5 converter output is also published as CF_DIBV5 because Windows native synthesis from a Mac CF_DIB can fail. The complete original stays in CF_DIBV5; standard sRGB BGRA screenshots get a40-byte BI_RGB CF_DIB with unchanged pixel rows for legacy readers. Other profiles/masks and older bitmap paths are preserved. MSWindowsClipboardNativeBitmapTests joins quick/exhaustive checks. Installed at dist/2026-09-16; backup .ltemp/package-before-clipboard-v5. Windows-only update; ChatGPT paste acceptance remains pending. See the active clipboard task.
...
- Game/app exclusion feature (Windows server only): see `docs/project/customizations.md` — settings key `server/excludedApps` in sibling `Deskflow-fch.conf` (separate from stock settings cleanup; legacy values migrate; explicit empty lists stay empty), watcher `MSWindowsForegroundWatcher` (events + 100 ms poll + pid snapshot + 300 ms resume debounce), hook-level pid guard, motion drop in `MSWindowsScreen`, 1 s watchdog. GUI: foreground label + `Excluded Apps` dialog (process picker with search) in the main window. BF6 policy restored and persistence fix installed on2026-09-22; backup `.ltemp/package-before-bf6-exclusion`. Gameplay acceptance unrun.
- Left Ctrl/Windows swap (Windows server): `server/leftCtrlSuperSwapScreen` in sibling `Deskflow-fch.conf` selects one canonical client name; legacy values migrate and explicit disabled targets survive stock-config cleanup. Restored/installed2026-09-22 with six persistence regressions; targeted4/4 pass and hidden quick57/59 in27.80s retains foreground/audio blockers. Physical Mac acceptance remains pending; local/right-side keys remain unchanged. Per-event physical modifier snapshots preserve shortcuts without changing the protocol. See `docs/project/customizations.md` section 5.
...
- macOS build uses user-local Qt6.10.3/GStreamer1.28.7 with `BUILD_STREAMING=ON`; all registered consumers compile. This supersedes the original Homebrew/OFF-only build. OBJC/OBJCXX languages are enabled in the root `CMakeLists.txt`; enabling them in a subdirectory breaks generation and recompiles `.m` files as ObjC++. C++-only declarations in headers shared with `.m` files need `#ifdef __cplusplus`. Install steps: see memory/mac install notes (macdeployqt, core `install_name_tool`, Developer ID signing).
...
### Historical Mac automatic acceptance update, superseded by coordinated native follow-up
...
### Coordinated Mac native follow-up, 2026-09-21

- Task-specific user authorization permits Mac Git/build/install despite the general user-only Git rule. Integrated source `e126d7fad7ca96a023d21c25c2d9638507143255` preserves Mac fixes and includes Windows capture teardown fix. Signed installed core identifies `e126d7fa`; prior bundle retained at `/Applications/Deskflow-pre-native-20260921.app`.
- Current Mac toolchain is Qt6.10.3/GStreamer1.28.7 with `BUILD_STREAMING=ON`, superseding the earlier OFF-only Mac notes. `build-validation` passed. Lifetime4/4, native launcher teardown3/3, acceptance7/7 passed. Quick40/48 in91.21s remains failing and over budget; full unrun. See `docs/project/macos-validation.md` for exact commands/failures.
- Native lock/unlock/sleep/wake probes confirm the private key appears only while locked on26.5.1/25F80; public on-console stays true. Private adapter proposal requires explicit approval and remaining session-switch validation. Product admission unchanged.
- Real shadow/opaque screenshot DIB conversion and installed-client bitmap export observed. Windows receive/paste and focus-return remain unverified. Settings/trust/certificate restored byte-identically; ordinary TLS reconnected after sleep. Installed capture needs Screen Recording authorization; streaming peer disconnected after wake. No operational media claim.

- Exclusion persistence validation (Windows): build `ExcludedAppsPersistenceTests`; `ctest --test-dir build/src/unittests -R ExcludedAppsPersistenceTests --output-on-failure`. Four isolated portable-process scenarios cover migration, stock-config cleanup, restart persistence, explicit removal and config-directory isolation. Included in quick/exhaustive checks. Latest hidden quick56/58 in27.76s retains the documented foreground/audio failures; targeted persistence/watcher/hook3/3 passes on the active desktop without creating visible windows.
