"""Run the owned native-control permission probe on an unselected Windows desktop.
...
parser.add_argument("--log", default="temp/proof-of-work/worklist-2026-09-13-1250/item-09/native-probe.txt")
parser.add_argument("--verify-core-guard", action="store_true")
args = parser.parse_args()
root = Path(__file__).resolve().parent.parent
log = (root / args.log).resolve()
log.parent.mkdir(parents=True, exist_ok=True)
probe = root / args.build / 'bin-streaming-validation/StreamingControlProbe.exe'
if not probe.exists():
    raise SystemExit('Build StreamingControlProbe first')
ctest = str(probe)
command = [ctest, '--fixture', '--output', str(log.parent), '--log', str(log)]
if args.verify_core_guard:
    command.append("--verify-core-guard")
...
try:
    ...
    if not kernel32.CreateProcessW(ctest, line, None, None, False, 0x08000000, None,
        ...
        raise ctypes.WinError(ctypes.get_last_error())
    print("Running owned control permission probe on unselected desktop " + name, flush=True)
