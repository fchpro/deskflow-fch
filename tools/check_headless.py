"""Run unchanged CTest binaries on an unselected Windows desktop.
...
parser.add_argument("--log", default="temp/proof-of-work/quick-check.txt")
parser.add_argument("--full", action="store_true", help="Include extended media/network tests")
...
if not ctest:
    raise SystemExit("ctest is not on PATH")
command = [ctest, "--test-dir", str((root / args.build / "src/unittests").resolve()),
           "--output-on-failure", "--output-log", str(log)]
if not args.full:
    command.extend(["-LE", "streaming-extended"])
...
try:
    ...
    if not kernel32.CreateProcessW(ctest, line, None, None, False, 0x08000000, None,
        ...
        raise ctypes.WinError(ctypes.get_last_error())
    print("Running unchanged CTest suite on unselected desktop " + name, flush=True)
